...
/**
 * A registry of MetaClass instances which caches introspection & reflection
 * information and allows methods to be dynamically added to existing classes at
 * runtime
 * 
...
public class MetaClassRegistryImpl implements MetaClassRegistry {
	 ...
	 *            method will be called to enable access to all methods when
	 *            using reflection
	 */
	...
	/**
	 * Looks for a class called
	 * 'groovy.runtime.metaclass.CustomMetaClassCreationHandle' and if it exists
	 * uses it as the MetaClassCreationHandle otherwise uses the default
	 * 
	...
	private void registerMethods(final Class theClass, final boolean useMethodWrapper, final boolean useInstanceMethods,
		...
		if (useMethodWrapper) {
			...
			try {
				...
				for (GeneratedMetaMethod.DgmMethodRecord record : records) {
					...
					MetaMethod method = new GeneratedMetaMethod.Proxy(record.className, record.methodName,
							ReflectionCache.getCachedClass(record.parameters[0]), record.returnType, newParams);
	 ...
	 * if oldMc is not null, then newMc will be used only if he stored mc is the
	 * same as oldMc
	 */
	...
	/**
	 * Sets a handle internally used to create MetaClass implementations. When
	 * replacing the handle with a custom version, you should reuse the old
	 * handle to keep custom logic and to use the default logic as fall back.
	 * WARNING: experimental code, likely to change soon
	 * 
	 * @param handle
	 *            the handle
	...
	public void removeMetaClassRegistryChangeEventListener(MetaClassRegistryChangeEventListener listener) {
		synchronized (changeListenerList) {
			...
			if (changeListenerList.size() == 0)
				changeListenerList.addFirst(first);
		}
	}

	/**
	 * Causes the execution of all registered listeners. This method is used
	 * mostly internal to kick of the listener notification. It can also be used
	 * by subclasses to achieve the same.
	 * 
	 * @param obj
	 *            object instance if the MetaClass change is on a per-instance
	 *            metaclass (or null if global)
	 * @param c
	 *            the class
	 * @param oldMC
	 *            the old MetaClass
	...
	public MetaClassRegistryChangeEventListener[] getMetaClassRegistryChangeEventListeners() {
		synchronized (changeListenerList) {
			return (MetaClassRegistryChangeEventListener[]) changeListenerList
					.toArray(new MetaClassRegistryChangeEventListener[changeListenerList.size()]);
		}
	}
	...
	/**
	 * Returns an iterator to iterate over all constant meta classes. This
	 * iterator can be seen as making a snapshot of the current state of the
	 * registry. The snapshot will include all meta classes that has been used
	 * unless they are already collected. Collected meta classes will be skipped
	 * automatically, so you can expect that each element of the iteration is
	 * not null. Calling this method is thread safe, the usage of the iterator
	 * is not.
	 * 
	 * @return the iterator.
	 */
