...
public class AntlrParserPlugin extends ASTHelper implements ParserPlugin, GroovyTokenTypes {
	...
	private void outputASTInVariousFormsIfNeeded(SourceUnit sourceUnit, SourceBuffer sourceBuffer) {
		...
		// output AST in format suitable for opening in
	...
	private void saveAsXML(String name, AST ast) {
		throw new RuntimeException("can not save as xml");
	...
	protected Parameter parameter(AST paramNode) {
		...
		if (node != null) {
			...
			if (isAnInterface()) {
				throw new ASTRuntimeException(node, "Cannot specify default value for method parameter '" + name + " = "
						+ rightExpression.getText() + "' inside an interface");
			}
		...
		if (firstParam)
			firstParamIsVarArg = variableParameterDef;
	...
	protected boolean setAccessTrue(AST node, boolean access) {
		if (!access) {
			...
			throw new ASTRuntimeException(node, "Cannot specify modifier: " + node.getText()
					+ " when access scope has already been defined");
		}
	}
	...
	protected Statement labelledStatement(AST labelNode) {
		...
		if (statement.getStatementLabel() == null) // if statement has multiple
													// labels, retain the last
													// one
	...
	protected Statement returnStatement(AST node) {
		...
		// This will pick up incorrect sibling node if 'node' is a plain
		// 'return'
		//
		// if (exprNode == null) {
	...
	protected Expression expressionSwitch(AST node) {
		...
		case GE:
			...
			/**
			 * TODO treble equal? return
			 * binaryExpression(Types.COMPARE_IDENTICAL, node);
			 * 
			 * case ???: return binaryExpression(Types.LOGICAL_AND_EQUAL, node);
			 * 
			 * case ???: return binaryExpression(Types.LOGICAL_OR_EQUAL, node);
			 */
	...
	/*
	 * commented out due to groovy.g non-determinisms protected Expression
	 * defaultMethodPointerExpression(AST node) { AST exprNode =
	 * node.getFirstChild(); String methodName = exprNode.toString();
	 * MethodPointerExpression methodPointerExpression = new
	 * MethodPointerExpression(null, methodName);
	 * configureAST(methodPointerExpression, node); return
	...
	protected Expression indexExpression(AST indexNode) {
		...
		BinaryExpression binaryExpression = new BinaryExpression(leftExpression, makeToken(Types.LEFT_SQUARE_BRACKET, bracket),
				rightExpression);
	...
	protected Expression binaryExpression(int type, AST node) {
		...
		if (Types.ofType(type, Types.ASSIGNMENT_OPERATOR)) {
			if (leftExpression instanceof VariableExpression || leftExpression.getClass() == PropertyExpression.class
					|| leftExpression instanceof FieldExpression || leftExpression instanceof AttributeExpression
					|| leftExpression instanceof DeclarationExpression || leftExpression instanceof TupleExpression) {
				// Do nothing.
			} else if (leftExpression instanceof ConstantExpression) {
				throw new ASTRuntimeException(node, "\n[" + ((ConstantExpression) leftExpression).getValue()
						+ "] is a constant expression, but it should be a variable expression");
				...
				if (!Types.ofType(lefttype, Types.ASSIGNMENT_OPERATOR) && lefttype != Types.LEFT_SQUARE_BRACKET) {
					throw new ASTRuntimeException(node, "\n" + ((BinaryExpression) leftExpression).getText()
							+ " is a binary expression, but it should be a variable expression");
				}
			} else if (leftExpression instanceof GStringExpression) {
				throw new ASTRuntimeException(node, "\n\"" + ((GStringExpression) leftExpression).getText()
						+ "\" is a GString expression, but it should be a variable expression");
			} else if (leftExpression instanceof MethodCallExpression) {
				throw new ASTRuntimeException(node, "\n\"" + ((MethodCallExpression) leftExpression).getText()
						+ "\" is a method call expression, but it should be a variable expression");
			} else if (leftExpression instanceof MapExpression) {
				throw new ASTRuntimeException(node, "\n'" + ((MapExpression) leftExpression).getText()
						+ "' is a map expression, but it should be a variable expression");
			} else {
				throw new ASTRuntimeException(node, "\n" + leftExpression.getClass() + ", with its value '" + leftExpression.getText()
						+ "', is a bad expression as the left hand side of an assignment operator");
			}
		}
		/*
		 * if (rightNode == null) { throw new
		 * NullPointerException("No rightNode associated with binary expression"
		 * ); }
		 */
		Expression rightExpression = expression(rightNode);
	...
	protected Expression methodCallExpression(AST methodCallNode) {
		...
		if (isType(LITERAL_super, selector)) {
			...
			configureAST(expression, methodCallNode);
			return expression;
		} else if (!implicitThis || isType(DYNAMIC_MEMBER, selector) || isType(IDENT, selector) || isType(STRING_CONSTRUCTOR, selector)
				|| isType(STRING_LITERAL, selector)) {
		...
		// if node text is found to be "super"/"this" when a method call is
		// being processed, it is a
		// call like this(..)/super(..) after the first statement, which
		// shouldn't be allowed. GROOVY-2836
		...
		// this=first statement in constructor is handled by
	...
	protected Expression gstring(AST gstringNode) {
		...
		for (AST node = gstringNode.getFirstChild(); node != null; node = node.getNextSibling()) {
			...
			case STRING_LITERAL:
				if (isPrevString)
					assertNodeType(IDENT, node); // parser bug
				isPrevString = true;
				text = node.getText();
	...
	private int getBoundType(AST node) {
		if (node == null)
			return -1;
		if (isType(TYPE_UPPER_BOUNDS, node))
			return TYPE_UPPER_BOUNDS;
		if (isType(TYPE_LOWER_BOUNDS, node))
			return TYPE_LOWER_BOUNDS;
		throw new ASTRuntimeException(node, "Unexpected node type: " + getTokenName(node) + " found when expecting type: "
				+ getTokenName(TYPE_UPPER_BOUNDS) + " or type: " + getTokenName(TYPE_LOWER_BOUNDS));
	}
	...
	protected ClassNode makeTypeWithArguments(AST rootNode) {
		...
		if (node == null || isType(INDEX_OP, node) || isType(ARRAY_DECLARATOR, node))
			return basicType;
	 ...
	 * imports, aliases or newly created classes
	 */
	/*
	 * protected String resolveTypeName(String name, boolean safe) { if (name ==
	 * null) { return null; } return resolveNewClassOrName(name, safe); }
	 */

	/**
	 * Extracts an identifier from the Antlr AST and then performs a name
	 * resolution to see if the given name is a type from imports, aliases or
