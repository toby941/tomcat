 ...
 * <li>dir
 * </ul>
 * When this task executes, it will recursively scan the dir and look for class
 * files to verify.
 */
	...
	private boolean readClass(String clazz) throws IOException {
		...
		for (int i = 0; i < methods.size(); ++i) {
			...
			if (method.instructions.size() > 0) {
				...
				/*
				 * = new TraceMethodVisitor(null) { public void visitMaxs(int
				 * maxStack, int maxLocals) { StringBuffer buffer = new
				 * StringBuffer(); for (int i = 0; i < text.size(); ++i) {
				 * String s = frames[i] == null ? "null" : frames[i].toString();
				 * while (s.length() < maxStack + maxLocals + 1) { s += " "; }
				 * buffer.append(Integer.toString(i + 100000).substring(1));
				 * buffer.append(" "); buffer.append(s); buffer.append(" : ");
				 * buffer.append(text.get(i)); } if (verbose)
				 * log(buffer.toString()); } };
				 */
