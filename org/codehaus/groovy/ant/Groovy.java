 ...
 * <p/>
 * <p>
 * Statements can either be read in from a text file using the <i>src</i>
 * attribute or from between the enclosing groovy tags.
 * </p>
 * 
 * @version $Id$
 */
public class Groovy extends Java {
	 ...
	 * <p/>
	 * Used to specify the debug output to print stacktraces in case something
	 * fails. TODO: Could probably be reused to specify the encoding of the
	 * files to load or other properties.
	 */
	...
	/**
	 * Should a new GroovyShell be used when forking. Special variables won't be
	 * available but you don't need Ant in the classpath.
	 * 
	 * @param useGroovyShell
	 ...
	 * automatically added to the classpath. Required unless statements are
	...
	/**
	 * Set an inline command to execute. NB: Properties are not expanded in this
	 * text.
	 * 
	 * @param txt
	...
	/**
	 * Returns a new path element that can be configured. Gets called for
	 * instance by Ant when it encounters a nested &lt;classpath&gt; element.
	 * 
	...
	public void execute() throws BuildException {
		...
		try {
			...
			try {
				...
				// Groovy tags
				// then read groovy statements in from a text file using the src
				// attribute
	...
	private void parseAndRunScript(GroovyShell shell, String txt, Object mavenPom, String scriptName, File scriptFile, AntBuilder builder) {
		try {
			...
			// not a script, try running through run method but properties will
			// not be available
			if (scriptFile != null) {
				try {
	...
	/**
	 * Setting to true will cause the contextClassLoader to be set with the
	 * classLoader of the shell used to run the script. Not used if fork is
	 * true. Not allowed when running from Maven but in that case the context
	 * classLoader is set appropriately for Maven.
	 * 
