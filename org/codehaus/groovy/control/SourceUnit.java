...
/**
 * Provides an anchor for a single source unit (usually a script file) as it
 * passes through the compiler system.
 * 
...
public class SourceUnit extends ProcessingUnit {
	...
	/**
	 * The pluggable parser used to generate the AST - we allow pluggability
	 * currently as we need to have Classic and JSR support
	 */
	...
	/**
	 * A descriptive name of the source unit. This name shouldn't be used for
	 * controlling the SourceUnit, it is only for error messages and to
	 * determine the name of the class for a script.
	 */
	...
	/**
	 * Returns the name for the SourceUnit. This name shouldn't be used for
	 * controlling the SourceUnit, it is only for error messages
	 */
	...
	/**
	 * Returns the Abstract Syntax Tree produced during convert()ing and
	 * expanded during later phases.
	 */
	...
	/**
	 * Convenience routine, primarily for use by the InteractiveShell, that
	 * returns true if parse() failed with an unexpected EOF.
	 */
	...
	/**
	 * A convenience routine to create a standalone SourceUnit on a String with
	 * defaults for almost everything that is configurable.
	 */
	...
	private void saveAsXML(String name, ModuleNode ast) {
		throw new RuntimeException("can not saveas xml");
	...
	/**
	 * Returns a sampling of the source at the specified line and column, of
	...
	/**
	 * This method adds an exception to the error collector. The Exception most
	 * likely has no line number attached to it. For this reason you should use
	 * this method sparingly. Prefer using addError for syntax errors or add an
	 * error to the {@link ErrorCollector} directly by retrieving it with
	 * getErrorCollector().
	 * 
	 * @param e
	...
	/**
	 * This method adds a SyntaxException to the error collector. The exception
	 * should specify the line and column number of the error. This method
	 * should be reserved for real errors in the syntax of the SourceUnit. If
	 * your error is not in syntax, and is a semantic error, or more general
	 * error, then use addException or use the error collector directly by
	 * retrieving it with getErrorCollector().
	 * 
	 * @param se
