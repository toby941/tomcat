...
public class EventAdapterGenerator {
	...
	static short BASECPCOUNT; // number of cp items + 1 ( cp item # 0 reserved
								// for JVM )
	static byte BASECP[]; //
	...
	static {
		logger = org.apache.juli.logging.LogFactory.getLog((org.apache.bsf.util.event.generator.EventAdapterGenerator.class).getName());
		...
		CLASSHEADER = ByteUtility.addBytes(CLASSHEADER, (short) 3); // minor
																	// version
		CLASSHEADER = ByteUtility.addBytes(CLASSHEADER, (short) 45); // major
																		// version

		// Start the constant pool for base items in all event adapter classes
		BASECPCOUNT = 17; // number of cp items + 1 ( cp item # 0 reserved for
							// JVM )
		...
		// cp item 10
		BASECP = Bytecode.addClass(BASECP, (short) 5); // Class
														// "java/lang/Object"

		// cp item 11
		BASECP = Bytecode.addClass(BASECP, (short) 6); // Class
														// "org/apache/bsf/util/event/EventAdapterImpl"

		// cp item 12
		BASECP = Bytecode.addClass(BASECP, (short) 7); // Class
														// "org/apache/bsf/util/event/EventProcessor"

		// cp item 13
		BASECP = Bytecode.addNameAndType(BASECP, (short) 2, (short) 1); // "<init>"
																		// "()V"

		// cp item 14
		BASECP = Bytecode.addNameAndType(BASECP, (short) 4, (short) 9); // "eventProcessor"
																		// "Lorg/apache/bsf/util/event/EventProcessor;"
		...
		FIXEDCLASSBYTES = ByteUtility.addBytes(FIXEDCLASSBYTES, (short) 0x21); // access_flags
																				// (fixed)
		FIXEDCLASSBYTES = ByteUtility.addBytes(FIXEDCLASSBYTES, (short) 20); // this_class
																				// (fixed)
		FIXEDCLASSBYTES = ByteUtility.addBytes(FIXEDCLASSBYTES, (short) 11); // super_class
																				// (fixed)
		FIXEDCLASSBYTES = ByteUtility.addBytes(FIXEDCLASSBYTES, (short) 1); // interface_count
																			// (fixed)
		FIXEDCLASSBYTES = ByteUtility.addBytes(FIXEDCLASSBYTES, (short) 19); // interfaces
																				// (fixed)
		FIXEDCLASSBYTES = ByteUtility.addBytes(FIXEDCLASSBYTES, (short) 0); // field_count
																			// (fixed)

		// initialization method, constructor
		INITMETHOD = ByteUtility.addBytes(INITMETHOD, (short) 1); // access_flags
		INITMETHOD = ByteUtility.addBytes(INITMETHOD, (short) 2); // name_index
																	// "<init>"
		INITMETHOD = ByteUtility.addBytes(INITMETHOD, (short) 1); // descriptor_index
																	// "()V"
		INITMETHOD = ByteUtility.addBytes(INITMETHOD, (short) 1); // attribute_count
		INITMETHOD = ByteUtility.addBytes(INITMETHOD, (short) 3); // attribute_name_index
																	// "Code"
		...
		INITMETHOD = ByteUtility.addBytes(INITMETHOD, (short) 16); // method_ref
																	// index
	...
	public static Class makeEventAdapterClass(Class listenerType, boolean writeClassFile) {
		...
		if (EVENTLISTENER.isAssignableFrom(listenerType)) {
			...
			String adapterClassName = CLASSPACKAGE
					+ (listenerTypeName.endsWith("Listener") ? listenerTypeName.substring(0, listenerTypeName.length() - 8)
			...
			/* ****************************************************************************************** */
			// optional inclusion of nonexceptional events affects exceptional
			// events indices
			...
			/* ****************************************************************************************** */
			// optional inclusion of exceptional events affects CP Items which
			// follow for specific methods
			...
			for (int i = 0; i < lms.length; ++i) {
				...
				if (eventName.equalsIgnoreCase("java/beans/PropertyChangeEvent")) {
					...
					if (0 == cpIndexPCE) {
						...
						constantPool = Bytecode.addNameAndType(constantPool, (short) (BASECPCOUNT + cpCount + 1), (short) (BASECPCOUNT
								+ cpCount + 2));
						constantPool = Bytecode.addMethodRef(constantPool, (short) (BASECPCOUNT + cpCount + 3), (short) (BASECPCOUNT
								+ cpCount + 4));
						cpCount += 6;
			...
			newClass = ByteUtility.addBytes(newClass, count); // constant_pool_count
																// (variable)
			newClass = ByteUtility.addBytes(newClass, BASECP); // constant_pool
																// (fixed)
			newClass = ByteUtility.addBytes(newClass, constantPool); // constant_pool
																		// (variable)
			newClass = ByteUtility.addBytes(newClass, FIXEDCLASSBYTES); // see
																		// FIXEDCLASSBYTES
																		// (fixed)
			newClass = ByteUtility.addBytes(newClass, (short) (lms.length + 1)); // method_count
																					// (variable)
			newClass = ByteUtility.addBytes(newClass, INITMETHOD); // constructor
																	// <init>
																	// (fixed)
			// methods
			...
			for (int i = 0; i < lms.length; ++i) {
				newClass = ByteUtility.addBytes(newClass, (short) 1); // access_flags
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (short) (cpBaseIndex + 3 * i + 0)); // name_index
																								// (variable)
				newClass = ByteUtility.addBytes(newClass, (short) (cpBaseIndex + 3 * i + 1)); // descriptor_index
																								// (variable)
				newClass = ByteUtility.addBytes(newClass, (short) 1); // attribute_count
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (short) 3); // attribute_name_index
																		// code(fixed)
				...
				newClass = ByteUtility.addBytes(newClass, (long) length); // attribute_length
																			// (variable)

				// start code attribute
				newClass = ByteUtility.addBytes(newClass, (short) 6); // max_stack
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (short) 3); // max_locals
																		// (fixed)
				...
				newClass = ByteUtility.addBytes(newClass, (long) length); // code_length
																			// (variable)

				// start code
				newClass = ByteUtility.addBytes(newClass, (byte) 0x2A); // aload_0
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0xB4); // getfield
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (short) 15); // index
																		// (fixed)

				if (propertyChangeFlag[i]) { // the propertyName is passed as
												// the first parameter
					newClass = ByteUtility.addBytes(newClass, (byte) 0x2B); // aload_1
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (byte) 0xB6); // invokevirtual
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (short) cpIndexPCE); // methodref
																					// (variable)
				} else { // the eventMethodName is passed as the first parameter
							// Target for method invocation.
					newClass = ByteUtility.addBytes(newClass, (byte) 0x12); // ldc
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (byte) (cpBaseIndex + 3 * i + 2)); // index
																									// (byte)
																									// (variable)
				}

				newClass = ByteUtility.addBytes(newClass, (byte) 0x04); // iconst_1
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0xBD); // anewarray
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (short) 10); // Class
																		// java/lang/Object
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0x59); // dup
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0x03); // iconst_0
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0x2B); // aload_1
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0x53); // aastore
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0xB9); // invokeinterface
																		// (fixed)

				// index to processEvent or processExceptionableEvent method
				length = 23; // actually an index into cp
				if (exceptionable && nonExceptionable) { // interface method
															// index
				...
				newClass = ByteUtility.addBytes(newClass, (short) length); // index
																			// (process??????...)
																			// (variable)

				newClass = ByteUtility.addBytes(newClass, (byte) 0x03); // iconst_0
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0x00); // noop
																		// (fixed)
				newClass = ByteUtility.addBytes(newClass, (byte) 0xB1); // return
																		// (fixed)

				if (exceptionable && 0 < excpIndex[i].length) { // exception
																// code
					newClass = ByteUtility.addBytes(newClass, (byte) 0x4D); // astore_2
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (byte) 0x2C); // aload_2
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (byte) 0xBF); // athrow
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (byte) 0x57); // pop
																			// (fixed)
					newClass = ByteUtility.addBytes(newClass, (byte) 0xB1); // return
																			// (fixed)
					// end code
					...
					newClass = ByteUtility.addBytes(newClass, (short) (1 + length)); // exception_table_length
																						// (variable)
					for (int j = 0; j < length; j++) { // catch exception types
														// and rethrow
						newClass = ByteUtility.addBytes(newClass, (short) 0); // start_pc
																				// (fixed)
						if (propertyChangeFlag[i]) {
							newClass = ByteUtility.addBytes(newClass, (short) 21); // end_pc
																					// (fixed)
							newClass = ByteUtility.addBytes(newClass, (short) 22); // handler_pc
																					// (fixed)
						} else {
							newClass = ByteUtility.addBytes(newClass, (short) 19); // end_pc
																					// (fixed)
							newClass = ByteUtility.addBytes(newClass, (short) 20); // handler_pc
																					// (fixed)
						}
						newClass = ByteUtility.addBytes(newClass, (short) excpIndex[i][j]); // catch_type
																							// (variable)
					}
					...
					if (propertyChangeFlag[i]) {
						newClass = ByteUtility.addBytes(newClass, (short) 21); // end_pc
																				// (fixed)
						newClass = ByteUtility.addBytes(newClass, (short) 25); // handler_pc
																				// (fixed)
					} else {
						newClass = ByteUtility.addBytes(newClass, (short) 19); // end_pc
																				// (fixed)
						newClass = ByteUtility.addBytes(newClass, (short) 23); // handler_pc
																				// (fixed)
					}
					if (nonExceptionable) {
				...
				newClass = ByteUtility.addBytes(newClass, (short) 0); // attribute_count
																		// (fixed)
				// end code attribute
