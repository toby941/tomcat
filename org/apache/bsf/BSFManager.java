...
/**
 * This class is the entry point to the bean scripting framework. An application
 * wishing to integrate scripting to a Java app would place an instance of a
 * BSFManager in their code and use its services to register the beans they want
 * to make available for scripting, load scripting engines, and run scripts.
 * <p>
 * BSFManager serves as the registry of available scripting engines as well.
 * Loading and unloading of scripting engines is supported as well. Each
 * BSFManager loads one engine per language. Several BSFManagers can be created
 * per JVM.
 * 
...
public class BSFManager {
	...
	// "abc" represents a dewey decimal number (three levels, each between 0 and
	// 9),
	// and "yyyy" a four digit year, "mm" a two digit month, "dd" a two digit
	// day.
	//
	// Example: "240.20060925" stands for: BSF version "2.4.0" as of
	// "2006-09-25"
	...
	protected ClassLoader classLoader = Thread.currentThread().getContextClassLoader(); // rgf,
																						// 2006-01-05
	...
	private Log logger = org.apache.juli.logging.LogFactory.getLog(this.getClass().getName());
	 ...
	 *         &quot;abc&quot; represents a dewey decimal number (three levels,
	 *         each between 0 and 9), and &quot;yyyy&quot; a four digit year,
	 *         &quot;mm&quot; a two digit month, &quot;dd&quot; a two digit day.
	 * 
	 * <br>
	 *         Example: &quot;<code>240.20061006</code>&quot; stands for: BSF
	 *         version <code>2.4.0</code> as of <code>2006-10-06</code>.
	 * 
	 * 
	 * @since 2006-01-17
	 */
	 ...
	 * @param funcBody
	 *            the multi-line, value returning script to evaluate
	 * @param paramNames
	 *            the names of the parameters above assumes
	 * @param arguments
	 ...
	 */
	public Object apply(String lang, String source, int lineNo, int columnNo, Object funcBody, Vector paramNames, Vector arguments)
			throws BSFException {
	 ...
	 *                if anything goes wrong while running the script
	 */
	public void compileApply(String lang, String source, int lineNo, int columnNo, Object funcBody, Vector paramNames, Vector arguments,
	 ...
	 *            language identifier
	 * @param source
	 *            (context info) the source of this script (e.g., filename)
	 * @param lineNo
	 *            (context info) the line number in source for script
	 * @param columnNo
	 *            (context info) the column number in source for script
	 * @param script
	...
	/**
	 * Declare a bean. The difference between declaring and registering is that
	 * engines are spsed to make declared beans "pre-available" in the scripts
	 * as far as possible. That is, if a script author needs a registered bean,
	 * he needs to look it up in some way. However if he needs a declared bean,
	 * the language has the responsibility to make those beans avaialable
	 * "automatically."
	 * <p>
	 * When a bean is declared it is automatically registered as well so that
	 * any declared bean can be gotton to by looking it up as well.
	 * <p>
	 * If any of the languages that are already running in this manager says
	 * they don't like this (by throwing an exception) then this method will
	 * simply quit with that exception. That is, any engines that come after
	 * than in the engine enumeration will not even be told about this new bean.
	 * <p>
	 * So, in general its best to declare beans before the manager has been
	 * asked to load any engines because then the user can be informed when an
	 * engine rejects it. Also, its much more likely that an engine can declare
	 * a bean at start time than it can at any time.
	 * 
	 * @param beanName
	 *            name to declare bean as
	 * @param bean
	 *            the bean that's being declared
	 * @param type
	 *            the type to represent the bean as
	 * 
	 * @exception BSFException
	 *                if any of the languages that are already running decides
	 *                to throw an exception when asked to declare this bean.
	 */
	...
	/**
	 * Execute the given script of the given language, attempting to emulate an
	 * interactive session w/ the language.
	 * 
	 * @param lang
	 *            language identifier
	 * @param source
	 ...
	 *         known to me (must have been registered via
	...
	public static String getLangFromFilename(String fileName) throws BSFException {
		...
		throw new BSFException(BSFException.REASON_OTHER_ERROR, "file extension missing or unknown: "
				+ "unable to determine language for '" + fileName + "'");
	}
	 ...
	 * @exception BSFException
	 *                if the language is unknown (i.e., if it has not been
	 *                registered) with a reason of REASON_UNKNOWN_LANGUAGE. If
	 *                the language is known but if the interface can't be
	 *                created for some reason, then the reason is set to
	 *                REASON_OTHER_ERROR and the actual exception is passed on
	 *                as well.
	 */
	public BSFEngine loadScriptingEngine(String lang) throws BSFException {
		...
		// except.
		try {
			Class engineClass = (classLoader == null) ? Class.forName(engineClassName) : classLoader.loadClass(engineClassName);
	...
	/**
	 * Registering a bean allows a scripting engine or the application to access
	 * that bean by name and to manipulate it.
	 * 
	 * @param beanName
	 ...
	 *            fully qualified name of the class interfacing the language to
	 *            BSF.
	 * @param extensions
	 *            array of file extensions that should be mapped to this
	 *            language type. may be null.
	 */
	...
	/**
	 * Set the classpath for those that need to use it. Default is the value of
	...
	/**
	 * Temporary directory to put stuff into (for those who need to). Note that
	 * unless this directory is in the classpath or unless the classloader knows
	 * to look in here, any classes here will not be found! BSFManager provides
	 * a service method to load a class which uses either the classLoader
	 * provided by the class loader property or, if that fails, a class loader
	 * which knows to load from the tempdir to try to load the class. Default
	 * value of tempDir is "." (current working dir).
	 * 
	 * @param tempDir
	...
	/**
	 * Undeclare a previously declared bean. This removes the bean from the list
	 * of declared beans in the manager as well as asks every running engine to
	 * undeclared the bean. As with above, if any of the engines except when
	 * asked to undeclare, this method does not catch that exception. Quietly
	 * returns if the bean is unknown.
	 * 
	 * @param beanName
	 ...
	 * @exception BSFException
	 *                if any of the languages that are already running decides
	 *                to throw an exception when asked to undeclare this bean.
	 */
