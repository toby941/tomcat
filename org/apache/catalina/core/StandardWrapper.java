...
public class StandardWrapper extends ContainerBase implements ServletConfig,
		...
		if (restrictedServlets == null) {
			...
			try {
				...
				if (is != null) {
					...
					log.error(sm
							.getString("standardWrapper.restrictedServletsResource"));
				}
			} catch (IOException e) {
				log.error(
						sm.getString("standardWrapper.restrictedServletsResource"),
						e);
			}
		}
		...
		if (!singleThreadModel) {
			...
			if (!singleThreadModel) {
				if (log.isTraceEnabled())
					log.trace("  Returning non-STM instance for custom classs");
	...
	protected boolean isServletAllowed(Object servlet) {
		...
		while (clazz != null
				&& !clazz.getName().equals("javax.servlet.http.HttpServlet")) {
			if ("restricted".equals(restrictedServlets.getProperty(clazz
					.getName()))) {
				return (false);
			}
