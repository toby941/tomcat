...
public class TextEditor extends JTextPane implements Pageable, Printable {
	...
	private static final Pattern TAB_BACK_PATTERN = Pattern.compile("^(([\t])|(    )|(   )|(  )|( ))", Pattern.MULTILINE);
	...
	public int print(Graphics graphics, PageFormat pageFormat, int page) throws PrinterException {
		if (page < numPages) {
			...
			// get the name of the pane (or user name) and the time for the
			// header
			...
			((Graphics2D) graphics).drawString(header, (float) (paper.getImageableWidth() / 2 - width / 2), (float) paper.getImageableY()
					/ 2 + fm.getHeight());
			...
			((Graphics2D) graphics).drawString(name, (float) (paper.getImageableWidth() / 2 - width / 2), (float) height - fm.getHeight()
					/ 2);

			((Graphics2D) graphics).translate(0, fm.getHeight());
			width = SwingUtilities.computeStringWidth(fm, pageStr);
			((Graphics2D) graphics).drawString(pageStr, (float) (paper.getImageableWidth() / 2 - width / 2),
	...
	protected void processKeyEvent(KeyEvent e) {
		...
		if (e.getID() == KeyEvent.KEY_RELEASED && e.getKeyCode() == KeyEvent.VK_INSERT && e.getModifiersEx() == 0) {
